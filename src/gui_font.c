...
int is_ttf_file(struct dirent* de)
{
	int d_namelen = strlen(de->d_name);
	if (d_namelen < 4) { return 0; }		// Cannot possibly have .ttf extension if name < 4
	if (!strcmp(de->d_name + d_namelen - 4, ".ttf")) { return 1; }
	return 0;
}

GUI_Font* GUI_Default_Font = NULL;
GUI_FontStyle GUI_Default_FontStyle = { GUI_BLACK, 24, TTF_STYLE_NORMAL };
GUI_Font* font_base = NULL;
int gui_load_fonts()
{
	struct dirent* de;
	DIR* dr = opendir(FONTS_DIR);
	int count = 0;

	if (!dr)
	{
		printf("Could not open font directory '%s'", FONTS_DIR);
		return count;
	}
	...
	while ((de = readdir(dr)) != NULL)
	{
		...
		sprintf(path_to_font, "%s/%s", FONTS_DIR, de->d_name);
		int d_namelen = strlen(de->d_name);

		GUI_Font* font = (GUI_Font*)malloc(sizeof(GUI_Font));							// 
		font->font_name = (char*)malloc(d_namelen - 3);								// Absolute
		memcpy(font->font_name, de->d_name, d_namelen - 4);
		//memcpy_s(font->font_name, de->d_namlen - 3, de->d_name, de->d_namlen - 4);		// Fuckery
		font->font_name[d_namelen - 4] = '\0';										//
