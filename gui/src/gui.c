...
void gui_draw_text(SDL_Renderer* renderer, GUI_Text* gui_text, unsigned char redraw)
{
	...
	SDL_Rect r = { gui_text->node->position_abs.x, gui_text->node->position_abs.y, 
...
{
	SDL_Rect r = { gui_button->node->position_abs.x, gui_button->node->position_abs.y,
...
{
	SDL_Rect r = { gui_text_input->node->position_abs.x, gui_text_input->node->position_abs.y,
...
void gui_draw()
{
	gui_get_hovered_node(active_state);
	gui_update_branch(active_state, active_state->base_node);
	// Debug lines
	if (active_state->hovered_node && 0)
	{
		SDL_Rect r = { active_state->hovered_node->position_abs.x, active_state->hovered_node->position_abs.y,
		...
		SDL_RenderDrawRect(active_state->renderer, &r);
		//printf("%p\n", state->hovered_node);
	}
}
//